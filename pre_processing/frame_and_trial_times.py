...
from pre_processing import read_tiffs_scanimage

# Get frame numbers and corresponding times, trial start times and frames - not including dark frames

def frame_and_trial_times_scanimage(data_path, metadata_file, overwrite = False, overwrite_daq_data = False, overwrite_tiff_data = False, n_trials_process = 0):
    ...
    if overwrite:
        ...
        # Get tiff metadata and mean pixel values
        tiff_data = read_tiffs_scanimage.main(data_path, metadata_file, daq_data, overwrite = overwrite_tiff_data, n_trials_process = n_trials_process)
